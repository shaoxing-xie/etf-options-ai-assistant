...
**自然语言 + 输出克制**：细则见 **`evolution_invariants.yaml` → `user_facing`** 与 **`docs/openclaw/execution_contract.md` §10`。用户可用**口语**下任务（不必每次粘贴长模板）；你**内部**仍须 read 三文件并守门禁。**对用户**默认短结论，**不要**复述契约全文或例行「请确认门禁」；**末尾**仍给完整 **8 行键值**供截取用。
