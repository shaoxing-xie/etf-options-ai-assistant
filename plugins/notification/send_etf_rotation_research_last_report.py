...
def tool_send_etf_rotation_research_last_report(
    *,
    ...
    try:
        ...
        return {"success": False, "message": f"failed read last rotation report: {e}", "data": {"path": str(p)}}
    ...
    return {"success": bool(isinstance(send_out, dict) and send_out.get("success")), "data": {"send": send_out}, "message": send_out.get("message") if isinstance(send_out, dict) else "unknown"}
