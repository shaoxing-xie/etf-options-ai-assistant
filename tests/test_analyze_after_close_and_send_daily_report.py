...
def test_composite_emits_run_quality_and_stage_timing_for_cron_balanced() -> None:
    from plugins.notification import send_daily_report as m

    ac = {
        "success": True,
        "message": "ok",
        "data": {"overall_trend": "偏多", "trend_strength": 0.7, "date": "2026-04-25"},
    }
    with patch("plugins.analysis.trend_analysis.tool_analyze_after_close", return_value=ac), patch(
        "plugins.notification.daily_report_normalization._maybe_autofill_cron_daily_market_p0",
        return_value=None,
        ...
        out = m.tool_analyze_after_close_and_send_daily_report(
            mode="test",
            workflow_profile="cron_balanced",
            stage_budget_profile="balanced",
            emit_stage_timing=True,
            max_concurrency=3,
        )

    assert out.get("success") is True
    data = out.get("data") or {}
    assert data.get("run_quality") in {"ok_full", "ok_degraded"}
    assert data.get("delivery", {}).get("success") is True
    assert data.get("delivery", {}).get("authority") == "toolResult.success"
    assert "stage_timing" in data
    assert "lineage_struct" in data

    rd = m_send.call_args.kwargs.get("report_data") or {}
    assert rd.get("run_quality") in {"ok_full", "ok_degraded"}
    assert isinstance(rd.get("stage_timing"), dict)
