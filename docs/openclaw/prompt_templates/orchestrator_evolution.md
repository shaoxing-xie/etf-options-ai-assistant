...
对用户：**默认**先用 **1～5 句中文**说明结论与下一步（不要用长篇「契约解读」或「请确认是否遵守…」问卷）。用户用**自然语言**下指令时，你已在内部 `read` 了 invariants / evolver_scope / execution_contract，**不要**把这三份文件的大段原文再贴给用户，除非用户明确要审计。

对机器：回复**末尾**须有一段**仅含**下列 8 行（可紧贴在上文人话摘要之后，单独成段），供日志截取：
