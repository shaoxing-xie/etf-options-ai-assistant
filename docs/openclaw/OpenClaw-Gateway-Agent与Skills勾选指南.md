...
| 2026-04-05 | `etf_main` / `etf_business_core_agent` 对齐分析向 Skill：`ota_etf_rotation_research`、`ota_strategy_research_loop`、`ota_quantitative_screening_brief`；`etf_notification_agent` 增加 `ota_market_regime_checklist`、`ota_historical_volatility_snapshot`；新增包 `ota-quantitative-screening-brief` |
