...
- **当前可用（推荐）**  
  ...
  - **自研 OpenClaw Skill（`ota-*`）**：源文件在仓库 [`skills/README.md`](../../skills/README.md)（索引与同步脚本）；主链路优先 `ota-strategy-fusion-playbook`、`ota-signal-risk-inspection`、`ota-daily-session-routine`；趋势三工具解读见 **`ota-trend-analysis-brief`**（与 `tool_analyze_after_close` / `before_open` / `opening_market` 对齐）。**`tool_quantitative_screening`** 的返回契约与叙事见 **`ota-quantitative-screening-brief`**。包数量以 **`skills/README.md` 索引表**（含 `SKILL.md` 的 `ota-*` 子目录）为准，并与 **`OpenClaw-Agent-ota-skills.md`** 中「覆盖性」条数一致维护。  
