...
- **规模（随仓库迭代）**：当前仓库 `skills/` 下自研 `ota-*` 含 `SKILL.md` 的子包以 [`skills/README.md`](../../skills/README.md) 索引表为准（另含 `ota-equity-factor-screening-brief`，`ota-quantitative-screening-brief` 保留为 deprecated 归档）。  
...
| 2026-04-19 | 选股工具迁移：`tool_screen_equity_factors` + `ota-equity-factor-screening-brief`；`ota-quantitative-screening-brief` deprecated |
