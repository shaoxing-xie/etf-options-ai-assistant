...
- **研究与长文（research.md 驱动）**  
  - ETF 轮动：管道版 `workflows/etf_rotation_research.yaml`（`tool_etf_rotation_research` → `tool_send_analysis_report`）；标的池与因子默认来自 **`config/rotation_config.yaml`** 与 **`config/symbols.json`**；Agent 版 `workflows/etf_rotation_research_agent.yaml`。策略回放：`workflows/strategy_research_playback.yaml`。涨停回马枪盘后：`workflows/limitup_pullback_after_close.yaml`（索引见 `workflows/README.md`）。
