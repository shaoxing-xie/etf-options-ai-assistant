...
4. **T2 L4 附录**：`ASSISTANT_INCLUDE_L4_SNAPSHOT=0` 关闭所有 T2 链路上的 `tool_l4_*` 附加块（核心正文与旧行为对齐）；契约字段见 `report_l4_snapshot_attachment_v1`。`data/meta/schema_registry.yaml`、`task_data_map.yaml`、`data_contract_version.json` **已纳入 Git**（`data/` 其余路径仍忽略，见根目录 `.gitignore`）。
