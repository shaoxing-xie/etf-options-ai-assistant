...
| `nightly_stock_screening.yaml` | 工作日 **20:00** | **夜盘选股**：`tool_screen_by_factors`（与 `tool_screen_equity_factors` 等价；默认 hs300）+ `tool_finalize_screening_nightly`（`data/screening/*.json`、质量门禁、观察池）；钉钉摘要；观察池见 `src/watchlist_storage.py` |
| `catalog_digest_inspect.yaml` | 周一 **8:15** | **Catalog 巡检**：`tool_plugin_catalog_digest` + `tool_summarize_attempts`（示例参数；生产替换 attempts JSON） |
